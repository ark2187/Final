...
We adjusted our approach accordingly by coding a "vehicle" and "non-vehicle" variable to understand the movement of people and goods as separately as possible.

We also knew that we wanted to visualize the crossings on a map. However, the U.S borders are long, especially when including Alaska, making it difficult to see all crossing info at once.

Using a shiny leaflet map solved this issue by allowing the user to select the US-Canada border or the US-Mexico border. The user can also select the type of crossing that is of interest to them. 

Users can also see the a boxplot of the monthly log number of crossings for each year and border to see state variations. Finally, a line graph of monthly crossing by vehicle type is included to show seasonality and vehicle trends. 
